...
/**
 * Hauptklasse Hier ist das Spiel implementiert, d.h. alles was während des
 * Spiels geschieht und alle Klassen, die währenddessen aufgerufen werden,
		...
		while (true) {
			...
			if (Globals.x > 1 || Globals.y > 1) // Ziel
			{
				if (Globals.room < 3) {
					Globals.quest.length = 0; // Damit wenn ein Raum ohne
												// Questgegenstände ist auch
												// keine Questgegenstände
												// gezeichnet werden.
			...
			} else if (Globals.x < 0 || Globals.y < 0) // Start
			{
				if (Globals.room > 1) {
					...
					Globals.x = Globals.zielx;
					Globals.y = Globals.ziely;
				} else // Sind in Raum 1. In Raum 1 geht es nicht zurück. Level
						// down soll nicht möglich sein
				{
